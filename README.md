...
###My Student edits, dealing with, as according to Moodle, the errors and bugs in my code
Currently, the following issues exist:
Cpplint erors:  Currently there exist errors from 1) using rand instead of rand_r, I decided it was more important to use rand and have it compile.
2) The "wierd number of spaces at line start" thing is stupid, because it throws a error if you change "public" to be indented the way it wants, the places where
it throws the error, it threw an error either way, both for the indent and lack thereof.  I couldn't figure out why they didn't line up right, as in the other files.
3) Sensor.h's header guard isn't getting read. your guess is as good as mine.

Jittery errors:  This is by far the largest issue with my code.  The collisions with both obstacles and walls, for both home_base and the robot, fail more times then they succeed, because of the jitter, which is caused by the robot overlapping with the thing it is colliding with, and multiple timesteps detect a collision. WHile this is a major flaw in my program, when it does not happen, the angles do reflect properly, although the jitter happens most of the time.

Doxygen erros: I don't know how well I did this doxygen stuff, but I did add comments in my code.  If the doxygen does not work, just go look at my code and you will see the comments I made.

NOTE: I put my documentation in the design doc as well as in mainpage.h.  I didn't understand which was preferred and I figured it wasnt a big deal.  for the changes to the REadme I changed home_base to be mobile and added it's motionbehaivor and motionhandler.  I did not add the color class because I didn't use it in my design and it seemed irrelevant. 
